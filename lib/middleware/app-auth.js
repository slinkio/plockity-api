...
var generator = module.exports = function ( refreshes ) {
  return function ( req, res, next ) {
    var token     = req.header('X-App-Authorization'),
        refreshes = ( refreshes === undefined ) ? true : refreshes;

    if( !token ) {
      return res.status(401).send('This resource requires the "X-App-Authorization" header with a fresh and relevant authorization\'s token');
    }
    ...
    Authorization.retrieve( token ).then(function ( appAuthorization ) {
      ...
      if( appAuthorization.isExpired ) {
        return res.status(401).send('Your authorization has expired');
      }
